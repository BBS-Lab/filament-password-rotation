...
use Illuminate\Foundation\Auth\User as NonRotatableUser;
...
beforeEach(function (): void {
    config([
        ...
        'filament-password-rotation.models' => [User::class, NonRotatableUser::class, 'App\\Does\\Not\\Exist'],
    ]);
});
...
it('skips models that do not implement the interface', function (): void {
    ...
    expect(runReport(all: true))->not->toContain(NonRotatableUser::class);
});
...
it('accepts a single model class name that is not wrapped in an array', function (): void {
    ...
    config(['filament-password-rotation.models' => User::class]);
...
it('reports nothing when no configured model yields rows', function (): void {
    config(['filament-password-rotation.models' => ['App\\Does\\Not\\Exist']]);
...
it('honours a one-day warning window', function (): void {
    $this->freezeTime();
    config(['filament-password-rotation.warn_days' => 1]);
...
it('keeps scanning models after skipping non-rotatable and missing ones (continue, not break)', function (): void {
    ...
    config(['filament-password-rotation.models' => [NonRotatableUser::class, 'App\\Does\\Not\\Exist', User::class]]);
