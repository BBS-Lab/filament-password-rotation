...
namespace BBSLab\FilamentPasswordRotation\Concerns;
...
trait RotatesPassword
{
    ...
    {
        return (string) config('filament-password-rotation.column', 'password_changed_at');
    }
    ...
    public function passwordExpiresAt(): ?CarbonInterface
    {
        return $this->passwordLastChangedAt()
            ?->copy()
            ->addDays((int) config('filament-password-rotation.days'));
    }
    ...
    public function passwordHasExpired(): bool
    {
        ...
        if ($expiresAt === null) {
            return (bool) config('filament-password-rotation.force_on_first_login');
        }
    ...
    public function passwordIsExpiring(): bool
    {
        ...
        if (! config('filament-password-rotation.enabled') || $this->passwordHasExpired()) {
            return false;
        }

        $warnDays = (int) config('filament-password-rotation.warn_days');
    ...
    public static function bootRotatesPassword(): void
    {
        static::creating(function (Model&MustRotatePassword $model): void {
            ...
            if (config('filament-password-rotation.force_on_first_login')) {
                return;
            }
        ...
        static::saved(function (Model&Authenticatable $model): void {
            ...
            if ((int) config('filament-password-rotation.history_count') > 0) {
